public void ReloadCal(DateTime ldate, int Selected) {
    ...
    DayOfWeek fdate = DayOfWeek.Monday;
    int idate = 1;
    int row = 1;
    ...
    while (true) {
        ...
        try {
            if (idate == (Date.DaysInMonth(ldate.Year, ldate.Month) + 1)) {
                break;
            }
        } catch() { }
    }
    ...
    private DayOfWeek GetFirstOfMonthDay(DateTime ThisDay) {
    ...
    private DayOfWeek ydate(DayOfWeek tday) {
        DayOfWeek rday;
        switch (tday) {
            case DayOfWeek.Sunday:
    ...
    private DayOfWeek tdate(DayOfWeek tday) {
        DayOfWeek rday;
        switch (tday) {
            case DayOfWeek.Sunday:
                rday = DayOfWeek.Monday;
